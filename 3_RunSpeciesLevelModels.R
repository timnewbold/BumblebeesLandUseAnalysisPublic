...
png("TempResults.png",width = 17.5,height = 14,units = "cm",res = 1200)
...
png("TempResultsInteractionTemperature.png",width = 17.5,height = 5,units = "cm",res = 1200)

par(mfrow=c(1,3))

xVals <- seq(
  from=quantile(m13$data$TEI_delta,0.025),
  to=quantile(m13$data$TEI_delta,0.975),
  length.out = 100)

nd <- data.frame(
  TEI_delta=xVals,
  occur=0,
  LandUse=factor("Natural",levels=levels(m13$data$LandUse)),
  TEI_BL=quantile(m13$data$TEI_BL,0.025),
  row.names = NULL)

preds.LowTEI.LUNatural <- PredictGLMER(model = m13$model,data = nd,se.fit = TRUE,seMultiplier = 1)
...
nd <- data.frame(
  TEI_delta=xVals,
  occur=0,
  LandUse=factor("Human",levels=levels(m13$data$LandUse)),
  TEI_BL=quantile(m13$data$TEI_BL,0.025),
  row.names = NULL)

preds.LowTEI.LUHuman <- PredictGLMER(model = m13$model,data = nd,se.fit = TRUE,seMultiplier = 1)
...
nd <- data.frame(
  ...
  occur=0,
  LandUse=factor("Natural",levels=levels(m13$data$LandUse)),
  TEI_BL=quantile(m13$data$TEI_BL,0.5),
  row.names = NULL)

preds.HighTEI.LUNatural <- PredictGLMER(model = m13$model,data = nd,se.fit = TRUE,seMultiplier = 1)
...
preds.HighTEI.LUHuman <- PredictGLMER(model = m13$model,data = nd,se.fit = TRUE,seMultiplier = 1)
...
nd <- data.frame(
  ...
  occur=0,
  LandUse=factor("Natural",levels=levels(m13$data$LandUse)),
  TEI_BL=quantile(m13$data$TEI_BL,0.975),
  row.names = NULL)
